...
struct Gemma4Q4BenchmarkTests {
    ...
    func mlxAlignedBenchmark() throws {
        ...
        print("=== \(Self.modelLabel) Q4 swift-lm benchmark (MLX-aligned) ===")
    ...
    func decodeKernelBreakdown() throws {
        ...
        print("\n=== \(Self.modelLabel) Decode Kernel Breakdown (aggressive, \(profiles.count) steps) ===")
    ...
    func endToEndBenchmark() throws {
        ...
        // For Q4, weight read per token is still roughly 1x total weight bytes
        // (every weight is read once per decode step via GEMV).
        let decodeBandwidth = Double(totalWeightBytes) * Double(generateCount) / decodeTime / 1e9

        var report = "=== Benchmark: \(Self.modelLabel) ===\n\n"
        report += "Memory Layout:\n"
        report += "  hidden:      \(String(format: "%6.1f", Double(hiddenBytes) / 1024)) KB\n"
        report += "  scratch:     \(String(format: "%6.1f", Double(scratchBytes) / 1024)) KB\n"
        report += "  residual:    \(String(format: "%6.1f", Double(residualBytes) / 1024)) KB\n"
        report += "  logits:      \(String(format: "%6.1f", Double(logitsBytes) / 1024)) KB\n"
        report += "  KV cache:    \(String(format: "%6.1f", Double(kvBytes) / 1024 / 1024)) MB\n"
        report += "  conv:        \(String(format: "%6.1f", Double(convBytes) / 1024)) KB\n"
        report += "  recurrent:   \(String(format: "%6.1f", Double(recurrentBytes) / 1024)) KB\n"
        report += "  per-layer:   \(b.perLayerInputs.map { String(format: "%.1f", Double($0.length) / 1024) + " KB" } ?? "none")\n"
        report += "  weights:     \(String(format: "%6.1f", Double(weightBytes) / 1024 / 1024)) MB\n"
        ...
        report += "\nBandwidth (estimate):\n"
        report += "  decode: \(String(format: "%.1f", decodeBandwidth)) GB/s (1× weight read per token)\n"
