...
struct PrefillRoutingContractTests {
    @Test("Prefill output head consumes final hidden source for all optimizers")
    func prefillOutputHeadConsumesFinalHiddenSourceForOptimizers() throws {
        ...
        for (name, optimizer) in optimizers {
            ...
            #expect(
                ...
                "\(name) optimizer prefill output head should read from finalHiddenSource buffer"
            )
            #expect(
                adjustedInputOffset == source.offset,
                "\(name) optimizer prefill output head offset mismatch: expected \(source.offset), got \(adjustedInputOffset)"
            )
        }
    }
}
