...
private func validateOperation(
    ...
    case .conditional(_, let thenRegion, let elseRegion):
        try validateRegion(thenRegion)
        try validateRegion(elseRegion)
    }
...
private func validateStructuralContract(_ op: Operation) throws {
    ...
    case .conditional(_, let thenRegion, let elseRegion):
        // Both regions must have same parameter and result arity as the operation
        if thenRegion.parameters.count != op.operands.count {
            throw GraphValidationError.residualArityMismatch(
                operandCount: op.operands.count,
                bodyParameterCount: thenRegion.parameters.count,
                operationKey: op.key
            )
        }
        if elseRegion.parameters.count != op.operands.count {
            throw GraphValidationError.residualArityMismatch(
                operandCount: op.operands.count,
                bodyParameterCount: elseRegion.parameters.count,
                operationKey: op.key
            )
        }
        if thenRegion.results.count != op.results.count {
            throw GraphValidationError.residualResultArityMismatch(
                resultCount: op.results.count,
                bodyResultCount: thenRegion.results.count,
                operationKey: op.key
            )
        }
        if elseRegion.results.count != op.results.count {
            throw GraphValidationError.residualResultArityMismatch(
                resultCount: op.results.count,
                bodyResultCount: elseRegion.results.count,
                operationKey: op.key
            )
        }
    }
