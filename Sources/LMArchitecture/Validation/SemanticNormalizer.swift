...
extension ForEach: _BuiltinComponent {
    fileprivate func _normalize(
        ...
        // Each child becomes a repeating(count: 1) to preserve layer index
        // tracking in the compiler. The compiler unrolls repeating(count: 1)
        // with layerIndex = 0, and ParameterResolver assigns the actual
        // layer index based on position in the sequence.
        var ops: [Operation] = []
        var current = upstream

        for child in children {
            let bodyParams = current.map { _ in ctx.freshValue() }
            ...
            let key = ctx.freshKey()
            let resultValues = current.map { _ in ctx.freshValue() }
            let op = Operation(
                key: key,
                kind: .repeating(count: 1, body: bodyRegion),
                operands: current.map { Operand(value: $0) },
                results: resultValues.map { OperationResult(id: $0) })
            ops.append(op)
...
extension LayerStack: _BuiltinComponent {
    fileprivate func _normalize(
        ...
        // Flat expansion: each child's operations are inlined sequentially.
        // ParameterResolver assigns layer indices by counting residual pairs.
        var ops: [Operation] = []
        var current = upstream
        for child in children {
            let fragment = try normalizeComponent(child, upstream: current, ctx: &ctx)
...
private func walkRegion(
    ...
    for (i, op) in region.operations.enumerated() {
        ...
        case .conditional(_, let thenRegion, let elseRegion):
            walkRegion(thenRegion, basePath: opPath.appending(.regionBody), keyLabels: keyLabels, entries: &entries)
            walkRegion(elseRegion, basePath: opPath.appending(.regionBranch(1)), keyLabels: keyLabels, entries: &entries)
        }
    }
}
